...
import { DisplayAddress } from '../models/DisplayAddress.interface';
...
const addAddress = async (newAddress: NewAddress): Promise<DisplayAddress | null> => {
  const response = await axios.post(
    `${process.env.REACT_APP_BASE_API}/auth/addAddress`,
    newAddress
  );
